...
def upload_one_project_zip(project_name, session, session_id, prod_zip_path):
    ...
    if response.status_code == requests.codes.ok:
        ...
        raise Exception("project:%s的zip包上传失败" % project_name)


def usage():
    print("\nThis is the usage function\n")
    print('Usage: python3 ' + sys.argv[0] + ' project_name')
    print('example: python3 upload_one_azkaban_project.py zdcs_spark-thriftserver')
    print('the zip file name must be project_name and end with .zip')
...
if __name__ == '__main__':
    try:
        ...
        if meta_dict.get(project_name) is None:
            logger.warn(
                "Azkaban项目名称在GP表dim_azkaban_info_dd_f中不存在!!!!!!!! "
                "可能是建完azkaban project后没有执行get_azkaban_project_meta.py脚本刷新GP表中的元数据")
        # Azkaban Web Url 登录认证
        session_id = azkabanutil.authenticate(session, azkaban_web_user, azkaban_web_password, azkaban_web_url)
        upload_one_project_zip(project_name, session, session_id, azkaban_zip_upload_abs_path)
